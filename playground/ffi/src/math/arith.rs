...
impl Field {
    pub fn new(modulus: u64) -> Result<Field, Error> {
        if modulus == 0 {
    ...
    ///(a+b)=(a+b)mod p
    pub fn add(&self, a: u64, b: u64) -> Result<U256, Error> {
    ...
    /// (a % m + m - b % m ) % m
    pub fn sub(&self, a: u64, b: u64) -> Result<U256, Error> {
        //(6-9)mod7=-3mod7
    ...
    pub fn add_inv(&self, a: U256) -> Result<U256, Error> {
        Ok((self.modulus - a) % self.modulus)
    }

    /// a * b = (a*b)mod p
    pub fn mult(&self, a: u64, b: u64) -> Result<U256, Error> {
        let formatted_a = U256::from(a);
        let formatted_b = U256::from(b);
    ...
    /// a^-1=[a^(p-a)]mod p
    pub fn mult_inv(&self, a: U256) -> Result<U256, Error> {
        if a == U256::from(0) {
            return Err(Error::InverseZero);
    ...
    pub fn div(&self, a: u64, b: u64) -> Result<U256, Error> {
        let formatted_a = U256::from(a);
        let formatted_b = U256::from(b);
    ...
    ///pow(a,b) = (a^b) mod p
    pub fn pow(&self, a: u64, power: u64) -> Result<U256, Error> {
        let formatted_a = U256::from(a);
